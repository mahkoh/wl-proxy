...
impl ZwpXwaylandKeyboardGrabManagerV1 {
    pub fn set_handler(&self, handler: impl ZwpXwaylandKeyboardGrabManagerV1Handler + 'static) {
...
impl ZwpXwaylandKeyboardGrabManagerV1 {
    ...
    pub fn send_destroy(
        ...
        if self.core.state.log {
            ...
            let args = format_args!("[{millis:7}.{micros:03}] server      <= zwp_xwayland_keyboard_grab_manager_v1#{}.destroy()\n", id);
    ...
    pub fn send_grab_keyboard(
        ...
        if self.core.state.log {
            let (millis, micros) = time_since_epoch();
            let args = format_args!("[{millis:7}.{micros:03}] server      <= zwp_xwayland_keyboard_grab_manager_v1#{}.grab_keyboard(id: zwp_xwayland_keyboard_grab_v1#{}, surface: wl_surface#{}, seat: wl_seat#{})\n", id, arg0_id, arg1_id, arg2_id);
            self.core.state.log(args);
        }
        let endpoint = &self.core.state.server;
        if !endpoint.flush_queued.replace(true) {
...
impl ProxyPrivate for ZwpXwaylandKeyboardGrabManagerV1 {
    fn new(state: &Rc<State>, version: u32) -> Rc<Self> {
        Rc::<Self>::new_cyclic(|slf| Self {
            core: ProxyCore::new(state, slf.clone(), ProxyInterface::ZwpXwaylandKeyboardGrabManagerV1, version),
    ...
    fn handle_request(self: Rc<Self>, client: &Rc<Client>, msg: &[u32], fds: &mut VecDeque<Rc<OwnedFd>>) -> Result<(), ObjectError> {
        ...
        match msg[1] & 0xffff {
            0 => {
                ...
                if self.core.state.log {
                    let (millis, micros) = time_since_epoch();
                    let args = format_args!("[{millis:7}.{micros:03}] client#{:<4} -> zwp_xwayland_keyboard_grab_manager_v1#{}.destroy()\n", client.endpoint.id, msg[0]);
                    self.core.state.log(args);
                }
            ...
            1 => {
                ...
                if self.core.state.log {
                    let (millis, micros) = time_since_epoch();
                    let args = format_args!("[{millis:7}.{micros:03}] client#{:<4} -> zwp_xwayland_keyboard_grab_manager_v1#{}.grab_keyboard(id: zwp_xwayland_keyboard_grab_v1#{}, surface: wl_surface#{}, seat: wl_seat#{})\n", client.endpoint.id, msg[0], arg0, arg1, arg2);
                    self.core.state.log(args);
                }
                let arg0_id = arg0;
                ...
                if let Some(handler) = handler {
                    ...
                    DefaultHandler.grab_keyboard(&self, arg0, arg1, arg2);
                }
            }
            n => {
                let _ = client;
