...
impl ExtSessionLockManagerV1 {
    ...
    pub fn send_destroy(
        ...
        if self.core.state.log {
            ...
            let args = format_args!("[{millis:7}.{micros:03}] server      <= ext_session_lock_manager_v1#{}.destroy()\n", id);
    ...
    pub fn send_lock(
        ...
        if self.core.state.log {
            let (millis, micros) = time_since_epoch();
            let args = format_args!("[{millis:7}.{micros:03}] server      <= ext_session_lock_manager_v1#{}.lock(id: ext_session_lock_v1#{})\n", id, arg0_id);
            self.core.state.log(args);
        }
        let endpoint = &self.core.state.server;
        if !endpoint.flush_queued.replace(true) {
    ...
    fn handle_request(self: Rc<Self>, client: &Rc<Client>, msg: &[u32], fds: &mut VecDeque<Rc<OwnedFd>>) -> Result<(), ObjectError> {
        ...
        match msg[1] & 0xffff {
            0 => {
                ...
                if self.core.state.log {
                    let (millis, micros) = time_since_epoch();
                    let args = format_args!("[{millis:7}.{micros:03}] client#{:<4} -> ext_session_lock_manager_v1#{}.destroy()\n", client.endpoint.id, msg[0]);
                    self.core.state.log(args);
                }
            ...
            1 => {
                ...
                if self.core.state.log {
                    let (millis, micros) = time_since_epoch();
                    let args = format_args!("[{millis:7}.{micros:03}] client#{:<4} -> ext_session_lock_manager_v1#{}.lock(id: ext_session_lock_v1#{})\n", client.endpoint.id, msg[0], arg0);
                    self.core.state.log(args);
                }
                let arg0_id = arg0;
