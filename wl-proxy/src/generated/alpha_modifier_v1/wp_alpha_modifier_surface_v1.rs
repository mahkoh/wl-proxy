...
impl WpAlphaModifierSurfaceV1 {
    pub fn set_handler(&self, handler: impl WpAlphaModifierSurfaceV1Handler + 'static) {
...
impl WpAlphaModifierSurfaceV1 {
    ...
    pub fn send_destroy(
        ...
        if self.core.state.log {
            ...
            let args = format_args!("[{millis:7}.{micros:03}] server      <= wp_alpha_modifier_surface_v1#{}.destroy()\n", id);
    ...
    pub fn send_set_multiplier(
        ...
        if self.core.state.log {
            let (millis, micros) = time_since_epoch();
            let args = format_args!("[{millis:7}.{micros:03}] server      <= wp_alpha_modifier_surface_v1#{}.set_multiplier(factor: {})\n", id, arg0);
            self.core.state.log(args);
        }
        let endpoint = &self.core.state.server;
        if !endpoint.flush_queued.replace(true) {
...
pub trait WpAlphaModifierSurfaceV1Handler: Any {
    ...
    fn destroy(
        &mut self,
        _slf: &Rc<WpAlphaModifierSurfaceV1>,
    ) {
    ...
    fn handle_request(self: Rc<Self>, client: &Rc<Client>, msg: &[u32], fds: &mut VecDeque<Rc<OwnedFd>>) -> Result<(), ObjectError> {
        ...
        match msg[1] & 0xffff {
            0 => {
                ...
                if self.core.state.log {
                    let (millis, micros) = time_since_epoch();
                    let args = format_args!("[{millis:7}.{micros:03}] client#{:<4} -> wp_alpha_modifier_surface_v1#{}.destroy()\n", client.endpoint.id, msg[0]);
                    self.core.state.log(args);
                }
            ...
            1 => {
                ...
                if self.core.state.log {
                    let (millis, micros) = time_since_epoch();
                    let args = format_args!("[{millis:7}.{micros:03}] client#{:<4} -> wp_alpha_modifier_surface_v1#{}.set_multiplier(factor: {})\n", client.endpoint.id, msg[0], arg0);
                    self.core.state.log(args);
                }
                if let Some(handler) = handler {
                    (**handler).set_multiplier(&self, arg0);
                } else {
                    DefaultHandler.set_multiplier(&self, arg0);
                }
            }
            n => {
                let _ = client;
