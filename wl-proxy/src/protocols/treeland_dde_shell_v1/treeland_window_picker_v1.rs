...
impl TreelandWindowPickerV1 {
    pub fn set_handler(&self, handler: impl TreelandWindowPickerV1Handler + 'static) {
            ...
            1 => {
                ...
                let arg0 = {
                    let Some(&len) = msg.get(offset) else {
                        return Err(ObjectError::MissingArgument("hint"));
                    };
                    offset += 1;
                    ...
                    if bytes.is_empty() {
                        return Err(ObjectError::NullString("hint"));
                    } else {
                        let Ok(s) = str::from_utf8(&bytes[..len-1]) else {
                            return Err(ObjectError::NonUtf8("hint"));
                        };
                        s
                    }
                };
