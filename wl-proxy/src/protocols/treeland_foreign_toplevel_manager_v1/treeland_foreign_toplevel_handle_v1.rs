...
impl TreelandForeignToplevelHandleV1 {
    /// Sets a new handler.
    pub fn set_handler(&self, handler: impl TreelandForeignToplevelHandleV1Handler) {
...
pub trait TreelandForeignToplevelHandleV1Handler: Any {
    ...
    fn handle_pid(
        &mut self,
        slf: &Rc<TreelandForeignToplevelHandleV1>,
        pid: u32,
    ) {
        if !slf.core.forward_to_client.get() {
            return;
        }
        let res = slf.try_send_pid(
            pid,
        );
        if let Err(e) = res {
    ...
    fn handle_event(self: Rc<Self>, msg: &[u32], fds: &mut VecDeque<Rc<OwnedFd>>) -> Result<(), ObjectError> {
        ...
        match msg[1] & 0xffff {
            ...
            1 => {
                ...
                let arg0 = {
                    let Some(&len) = msg.get(offset) else {
                        return Err(ObjectError(ObjectErrorKind::MissingArgument("title")));
                    };
                    offset += 1;
                    ...
                    if bytes.is_empty() {
                        return Err(ObjectError(ObjectErrorKind::NullString("title")));
                    } else {
                        let Ok(s) = str::from_utf8(&bytes[..len-1]) else {
                            return Err(ObjectError(ObjectErrorKind::NonUtf8("title")));
                        };
                        s
                    }
                };
            ...
            6 => {
                ...
                let arg0 = {
                    let Some(&len) = msg.get(offset) else {
                        return Err(ObjectError(ObjectErrorKind::MissingArgument("state")));
                    };
                    offset += 1;
                    let len = len as usize;
                    let words = ((len as u64 + 3) / 4) as usize;
            ...
            9 => {
                ...
                let arg0 = if arg0 == 0 {
                    ...
                    let Ok(arg0) = (arg0 as Rc<dyn Any>).downcast::<TreelandForeignToplevelHandleV1>() else {
                        let o = self.core.state.server.lookup(arg0_id).unwrap();
                        return Err(ObjectError(ObjectErrorKind::WrongObjectType("parent", o.core().interface, ObjectInterface::TreelandForeignToplevelHandleV1)));
                    };
                    Some(arg0)
                };
                let arg0 = arg0.as_ref();
